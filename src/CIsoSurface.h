...
public:
	...
	void GenerateSurface(const std::vector<T> &ptScalarField,
						 T tIsoLevel,
	...
	int GetVolumeLengths(float &fVolLengthX,
						 float &fVolLengthY,
						 float &fVolLengthZ);

	// output reconstruction result
	void OutputMesh(const pcl::PointXYZ &oOffset,
...
protected:
	...
	POINT3DID CalculateIntersection(unsigned int nX,
									unsigned int nY,
									unsigned int nZ,
