...
public:
	...
	void ConvertCloud(const pcl::PointCloud<pcl::PointXYZ> &vSCloud,
					  pcl::PointCloud<pcl::PointXYZ> &vTCloud);

	// build the convex hull to the index of the input
	void IndexFromHulltoInput(pcl::PointCloud<pcl::PointXYZ>::Ptr &pHullVertices);

	// compute visibility
	void Compute(const pcl::PointCloud<pcl::PointXYZ>::Ptr &pCloud,
				 bool bIndexRelation = true);
	...
	// polygon of convex hull. The faces of hull which idx is about hull rather than original point cloud.
	std::vector<pcl::Vertices> m_vHullPolygonIdxs;

	// vertices / points of convex hull
	// Note that, Unable to ensure that the point order of the convex hull points and the original point 
	// cloud is the same after PCL function. So we need to established the correspondence between them.
	pcl::PointCloud<pcl::PointXYZ>::Ptr m_pHullVertices;

	// view point in world coordinate
	pcl::PointXYZ m_oViewPInWorld;

	// viewpoint idx in original point cloud
	int m_iViewWorldIdx;

	// Hull To Local idx. 
	// The index of the point on the convex hull relative to the original point cloud
