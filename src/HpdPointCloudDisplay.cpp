...
//手动制定颜色，先显示一些东西
	...
	std::cout<<"we have "<<classnumber+1<<" classes"<<std::endl;
	// std::cout<<"请问大人，您要上什么颜色? 请输入序号"<<std::endl;
	std::cout<<"sir, what color do u want, please just give me a number, alright?"<<std::endl;
	displaytable();
   //手动赋值
	for(int i=0;i<classnumber+1;i++){
		// std::cout<<"类"<<i+1<<"颜色序号，请输入并回车"<<std::endl;
		std::cout<<"class "<<i+1<<" color #,pls input a number, and then push 'Enter'"<<std::endl;
...
void HpdDisplay::displaytable(){
	...
	std::cout<<"      color tabel as fellow: "<<std::endl;

	for(size_t i=0;i<colors.size();i++)
		// std::cout<<"序号"<<i<<"是"<<colors[i].name<<std::endl;
		std::cout<<"No."<<i<<" is "<<colors[i].name<<std::endl;
