...
========================================*/
GHPR::GHPR(pcl::PointXYZ f_viewpoint, double f_param) : m_pTransCloud(new pcl::PointCloud<pcl::PointXYZ>),
...
Function: geometric inversion transformation, it set m_oViewPInWorld as origin, and converts points from world into local coordiante.
========================================*/
void GHPR::ConvertCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr &pCloud)
{
	...
	// the memory location of the original point cloud is used for storage.
...
void GHPR::Compute(const pcl::PointCloud<pcl::PointXYZ>::Ptr &pCloud, bool bIndexRelation)
{
	...
	// input the point clouds of m_pTransCloud
	oConvexHull.setInputCloud(m_pTransCloud);

	// set the dimension
	oConvexHull.setDimension(3);

	// output: pHullVertices - the vertices(points) of convex hull
	// output: m_vHullPolygonIdxs - the faces of convex hull (vertice indexes of a face)
	...
	if (bIndexRelation)
	{
		...
		// Actually, a reletionship between m_pHullVertices and m_pTransCloud. for each point in m_pHullVertices,
		// we find the its correspondence idx in m_pTransCloud. we put the relationship in m_vHullInInputIdx.
		IndexFromHulltoInput(m_pHullVertices);

		// the index relationship has been established, so we set it True.
